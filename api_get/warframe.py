...
# GET request to API, saves them in instance variables:
class WF_init:
...
class ProgenitorCommand:
    ...
    async def qpg(self, entry):  # returns an embed to user containing progenitor info
        entry = entry.strip()
...
# if image isn't available in API, scrape it instead
async def wiki_image(img):
    ...
    sidebar = soup.find(
        "aside",
        class_="portable-infobox pi-background pi-border-color pi-theme-wikia pi-layout-default",
    )
...
async def mod(var):
    ...
    try:
        if wf_init.wf_mod[num]["name"] == entry_mod:
            if wf_init.wf_mod[num].get("drops") == None:
                embed.add_field(
                    name="Showing the first 10 locations:",
                    value=f"```Drop table data unavailable```",
                )
                return embed
            else:
                display.append("```")
    ...
    embed.add_field(
        name="Showing the first 10 locations:", value=f"{escape.join(display)}"
    )
    return embed
    ...
    try:
        if wf_init.wf_wep[num]["name"] == entry:
            for j in wf_init.wf_wep[num]["attacks"]:
                ...
                embed.add_field(
                    name=f"Damage: {total_dmg}", value=f"{escape.join(display)}"
                )
...
# returns an embed to the user containing frame info
async def frame(var):
