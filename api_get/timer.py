...
async def timer():
    ...
    if vallis_state == False:
        embed.add_field(name="Plains of Eidolon", value=f"*{cetus_state}*\nExpires in {cetus_timeLeft}", inline=False)

        embed.add_field(name="Orb Vallis", value=f"*Cold*\nExpires in {vallis_timeLeft}", inline=False)

        embed.add_field(name="Cambion Drift", value=f"*{(cambion_active).title()}*\nExpires in: {cambion_timeLeft}", inline=False)
        return embed

    else:
        embed.add_field(name="Plains of Eidolon", value=f"*{cetus_state}*\nExpires in {cetus_timeLeft}", inline=False)

        embed.add_field(name="Orb Vallis", value=f"*Warm*\nExpires in {vallis_timeLeft}", inline=False)
